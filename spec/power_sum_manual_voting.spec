...
definition summedFunction(env e, address u) returns uint256 = getPowerCurrent(e, u, VOTING_POWER());

// an invariant to assume, proven separately
definition delegate_and_count_inv(address u, uint8 type) returns bool =
	(u != 0 && getDelegateeByType(u, type) != 0 && getDelegateeByType(u, type) != u) => ((type == VOTING_POWER() => _votingPowerSnapshotsCounts(u) > 0) && (type == PROPOSITION_POWER() => getPropositionPowerSnapshotCount(u) > 0));
...
rule changesOne(method f, address Addr1, address Addr2)
{
	...
	require delegate_and_count_inv(Addr1, 0);
	require delegate_and_count_inv(Addr2, 0);
...
rule changesTwo(method f, address Addr1, address Addr2, address Addr3)
{
	...
	require delegate_and_count_inv(Addr1, 0);
	require delegate_and_count_inv(Addr2, 0);
	require delegate_and_count_inv(Addr3, 0);
